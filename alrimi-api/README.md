...
**예약 시각이 되면 n8n 크론이 매시 보낸다** — 웹 푸시가 먼저, 못 닿은 것만 ntfy 로
(아래 "크론이 부르는 자리"). 상세 화면의 "보내기"로 하나를 손으로 밀 수도 있다.
...
나오고, 일정이 목록·달력·상세·검색에 함께 나오며 알림(매시·주간·손으로 보내기)도 받는
사람마다 따로 간다. **보기와 알림만이다.** 일정 등록·수정·삭제, 공간 이름·색·함께 보기
변경은 주인만이다(403). 일정 응답의 `can_edit` 이 그 선을 웹에 알린다. 구글 캘린더 복사는
주인 것만 담는다.
...
제목은 한 건이어도 건수만 적는다(`[어린이집] 일정 1건`). 무엇인지는 본문이 말한다 —
여러 건일 때와 모양이 같다. 날짜 줄에는 요일이 붙는다(`2026-09-17 (목)`).
