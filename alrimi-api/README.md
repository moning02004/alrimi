...
| GET | `/events/alerts` | 매시. 시각이 된 예약을 웹 푸시로 **보내고**, 못 닿은 것만 돌려준다 |
| PATCH | `/events/alerts/status` | 밀어 보낸 뒤. `{"ids": [...]}` 를 발송으로 찍는다 |

**`GET /events/alerts` 는 읽기만 하는 자리가 아니다.** 부르면 그 자리에서 웹 푸시가
나간다. 웹 푸시는 본문을 기기의 공개키로 암호화해야 해서(VAPID + aes128gcm) n8n 이
대신 쏘지 못하고 — 그 열쇠는 이 서버에만 있다 — 조회하는 자리가 발송까지 겸한다.
크론을 한 번 더 부르게 해서 얻을 것이 없다.

그래서 매시 한 바퀴는 이렇게 돈다.

```
GET /events/alerts          ← 이 서버가 웹 푸시를 보낸다
  → data: 못 닿은 묶음만    → n8n 이 이것만 ntfy 로 발행
  → ids:  묶기 전 예약 전부 → PATCH /events/alerts/status
```

`ids` 에는 **웹 푸시로 이미 닿은 것도 담긴다.** 발송으로 찍힐 것은 어느 길로 갔든
같아서다. 여기서 빼면 웹 푸시로 받은 예약이 pending 으로 남아 다음 시간에 ntfy 로
한 번 더 나간다.

`data` 가 비어 오는 것은 정상이다 — 모두가 브라우저 알림으로 받았다는 뜻이다.
n8n 의 ntfy 노드는 빈 목록에서 아무것도 안 하도록 두면 된다.
