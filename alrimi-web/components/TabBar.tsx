...
export function TabBar() {
  ...
  return (
    // 아래 탭바는 모바일 전용. PC(lg 이상)에서는 왼쪽 SideNav 가 대신한다
    <nav className="safe-bottom mt-auto border-t border-line bg-card lg:hidden">
      <div className="flex h-14 items-center px-2">
        ...
        <div className="flex-1 text-center">
          <button
            onClick={() => openAdd()}
            aria-label="일정 등록"
            // PC 에서는 마우스를 올리면 단축키를 알려준다
            title="일정 등록 (N)"
            className="mx-auto flex h-11 w-11 items-center justify-center rounded-full
                       bg-pine text-white transition-colors hover:bg-pine/90 active:bg-pine/90"
          >
...
function Tab({
  ...
  return (
    <Link
      ...
      title={label}
      className="group flex flex-1 items-center justify-center self-stretch"
    >
      <Icon
        className={`h-6 w-6 transition-colors ${
          active ? "text-pine" : "text-muted group-hover:text-ink"
        }`}
        aria-hidden="true"
      />
    </Link>
